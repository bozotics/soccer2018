...
void setmove(float angle, float speed, float angleSpin, float multSpin) {
	...
	if(multSpin == 0) correction = getAngle * cmps_mult;
	else correction = (-1)*(angleSpin - getAngle)  * multSpin;
	fl_pwm -= correction;
	fr_pwm += correction;
	bl_pwm -= correction;
	br_pwm += correction;
...
void goalieAlignBall(float yBall, float goal[2][4], float backUltra, float yLeeway, float ultraLeeway, float goalLeeway){
	backUltraDiff = backUltraThreshold - backUltra;
	backUltraCorr = fabsf(backUltraDiff*backUltraMult);
	yDiff = yBall-228;
	yCorr = fabsf(yDiff*yMult);
	if (goal[side][1]>(228-goalLeeway)){
		setmove(90, (goal[side][1]-228+goalLeeway)*goalMult, 0, 0);
		Serial.println("rightgoal");
	}
	else if (goal[side][0]<(228+goalLeeway)){
		setmove(270, fabsf(goal[side][0]-228-goalLeeway)*goalMult, 0, 0);
		Serial.println("leftgoal");
	}
		...
		Serial.println("leftball");
	}
	else if (yDiff < -yLeeway){
		setmove(90, fabsf(yCorr), 0, 0);
		Serial.println("rightball");
	}
